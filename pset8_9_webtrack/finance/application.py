...
def index():
    ...
    rows = db.execute(
        "SELECT cash, username FROM users WHERE id=:user_id", user_id=session["user_id"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        get_portfolio = db.execute(
            "SELECT symbol, name, shares, price FROM stocks WHERE user_id=:user_id", user_id=session["user_id"])
...
def sell():
    ...
    if request.method == "GET":
        ...
        symbols = db.execute(
            "SELECT symbol from stocks WHERE user_id=:user_id", user_id=session["user_id"])
